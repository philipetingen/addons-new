...
class AccountPaymentTerm(models.Model):
    ...
    def compute_with_milestones(self, reference_dates, total_amount, context=None):
        ...
        Args:
            reference_dates: Dict of available dates (keys = milestone codes)
        ...
        for line in self.line_ids.sorted('sequence'):
            ...
            if line.milestone_mode == 'milestone' and line.milestone_type_id:
                # ✅ FIXED: Use milestone code to lookup date from reference_dates dict
                milestone_code = line.milestone_type_id.milestone_code
                milestone_date = reference_dates.get(milestone_code)
    ...
    def get_payment_schedule_for_deal(self, deal):
        ...
        Note: Uses getattr() for flexibility - works with any object that has date fields.
        ...
        if context['type'] == 'sales':
            total_amount = getattr(deal, 'amount_untaxed_sale', 0) or getattr(deal, 'total_value', 0) or getattr(deal, 'total_sale_amount', 0)
        else:
            total_amount = getattr(deal, 'amount_untaxed_purchase', 0) or getattr(deal, 'purchase_total', 0) or getattr(deal, 'total_purchase_amount', 0)
